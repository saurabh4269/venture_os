...
Each company also has `FIXTURE_ONLY-<slug>-FY26-M1-M6-history.xlsx` (Apr–Sep 2025 / FY26 M1–M6) for runway, revenue trend, and Compare. Closing cash is set so Command runway (cash ÷ avg last-3 burns) spreads from ~1.6 mo (Salad Days) through ~34 mo (Yepoda). Deconstruct latest cash is `—` (missing ≠ 0). Regenerated by `node scripts/generate-fixture-history.mjs`.
