...
export const FLAG_CATALOG: { key: FlagKey; label: string; defaultThreshold: number }[] = [
  ...
  { key: "plan_variance", label: "Plan variance", defaultThreshold: 0.15 },
...
export function detectPlanVariance(actual: Num, plan: Num, threshold = 0.15): FlagHit | null {
  ...
  if (!isPresent(v) || Math.abs(v) < threshold) return null;
  return {
    flagKey: "plan_variance",
    severity: Math.abs(v) > 0.3 ? "high" : "med",
    evidence: { actual, plan, variance: v, threshold },
  };
}
...
export function detectAll(input: {
  ...
  return [
    detectRunwayShort(input.cash, input.burn),
    ...
    detectMisLate(input.lastMisPeriodEnd, asOf),
