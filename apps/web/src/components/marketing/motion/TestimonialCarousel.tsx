...
/** Methodology quotes only — role titles, no invented customer names. */
export function TestimonialCarousel() {
  ...
  const go = useCallback(
    (next: number) => {
      setDirection(next > active ? 1 : next < active ? -1 : 1);
      setActive(next);
    },
    [active],
  );
  ...
  return (
    ...
    >
      ...
      <p className="mkt-testimonial-live" aria-live="polite">
        {quote.role}
      </p>
      ...
      <div className="mkt-arc-controls">
        ...
        <button
          type="button"
          className="mkt-arc-btn mkt-arc-btn-wide"
          aria-pressed={paused}
