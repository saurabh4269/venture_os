...
export default function FlagsPage() {
  ...
  const flagsKey = useMemo(() => {
    ...
    return `/api/flags?${p.toString()}`;
  }, [status, severity, companyId, flagKey]);

  const { data: raw, error, mutate } = useSWR<FlagPayload>(flagsKey, bookFetcher);
  const data = raw ?? { flags: [], sourceRefs: [] };
  ...
  useEffect(() => {
    if (!raw) return;
    setOpenId((cur) => {
      if (cur && raw.flags.some((f) => f.id === cur)) return cur;
      return raw.flags[0]?.id ?? null;
    });
  }, [raw]);
  ...
  async function recompute() {
    ...
    try {
      ...
      setActionErr(e instanceof Error ? e.message : "Recompute failed");
    } finally {
      setBusy(false);
    }
  }
